/**
 * Spider board scene. Renders canonical `SpiderState` and dispatches
 * `Move`s on interaction. Ten columns with compact stacking; a card is
 * grabbable only when the run from it down is same-suit descending
 * (display concern — the engine validates the drop itself).
 */
import Phaser from 'phaser';
import type { Card, GameState, Move, PileRef, SpiderState } from '../../engine/types.js';
import { gameStore } from '../../stores/gameStore.svelte.js';
import { computeSpiderLayout, type SpiderLayout } from '../spiderLayout.js';
...
/** Whether `col[idx..]` is grabbable: face-up, descending, one suit. */
...
export class SpiderScene extends Phaser.Scene implements DragHost {
  private sprites = new Map<string, CardSprite>();
  private piles: Pile[] = [];
  private layout!: SpiderLayout;
  private current?: SpiderState;
  ...
  create(): void {
    ...
    if (gameStore.state.variant !== 'spider') gameStore.newGame('spider');
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => this.dispose());
  }

  // ---- DragHost ----

  /** The grabbed card plus every card on top of it (a same-suit run). */
  ...
  private rebuildLayout(): void {
    this.layout = computeSpiderLayout(this.scale.width, this.scale.height);
  ...
  private computeTargets(s: SpiderState): Target[] {
    const L = this.layout;
    const out: Target[] = [];
    ...
    // Cleared runs render as their King top card only — 8 slots max.
    ...
    s.tableau.forEach((col, ci) => {
      ...
      // their index; deeper face-up cards collapse to rank-only strips.
      let prevUp = -1;
      ...
      col.forEach((c, i) => {
        ...
        out.push({
          ...
          y,
          depth: 300 + ci * 40 + i,
          interactive: isOrderedRun(col, i) ? 'drag' : 'none',
          dealOrder: i * 10 + ci // column-major, like a real spider deal
        });
      });
    });
    return out;
  ...
  private syncState(s: SpiderState): void {
